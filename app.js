...
async function loadData(){ const res = await fetch('/data/data.json'); const data = await res.json(); return data; }
...
function headerControls(){ const h = document.querySelector('header'); const wrap = el('div', {class:'small'}, []); const modeSel = el('select', {}, [ el('option',{value:'all'},'Alle opgaver'), el('option',{value:'units'},'Kun enheder') ]); modeSel.value = CONFIG.mode; modeSel.onchange = () => { CONFIG.mode = modeSel.value; }; const chk = el('input',{type:'checkbox'}); chk.checked = CONFIG.strictUnits; chk.onchange = () => { CONFIG.strictUnits = chk.checked; };
  function mkWeightInputs(label, arr){ const w = el('span',{}); w.appendChild(el('span',{style:'margin:0 6px;'},label+':')); const e = el('input',{type:'number',value:arr[0],min:'0',max:'8',style:'width:48px'}); const m = el('input',{type:'number',value:arr[1],min:'0',max:'8',style:'width:48px;margin-left:4px'}); const hI = el('input',{type:'number',value:arr[2],min:'0',max:'8',style:'width:48px;margin-left:4px'}); e.onchange = () => arr[0]=Number(e.value); m.onchange = () => arr[1]=Number(m.value); hI.onchange = () => arr[2]=Number(hI.value); w.appendChild(e); w.appendChild(m); w.appendChild(hI); return w; }
...
function renderSubjects(data){ const container = document.getElementById('subjects'); container.innerHTML=''; container.appendChild(el('h2',{},'Emner')); const grid = el('div',{class:'grid'}); Object.entries(data).forEach(([key, subject])=>{ const card = el('div',{class:'card'},[ el('h3',{},subject.title), el('p',{class:'small'},`Niveauer: 1–${subject.levels.length}`), el('a',{href:'#',class:'button',onclick:(ev)=>{ev.preventDefault();renderLevels(key, subject);}},'Se niveauer') ]); grid.appendChild(card); }); container.appendChild(grid); }
...
function pickAdaptive(tasks, level){ if(CONFIG.mode==='units'){ tasks = tasks.filter(t=>t && (t.unit || (t.tags&&t.tags.includes('unit')))); } const easy = tasks.filter(t=>t.difficulty==='easy'); const med = tasks.filter(t=>t.difficulty==='medium'); const hard = tasks.filter(t=>t.difficulty==='hard'); let needEasy, needMed, needHard; if(level <=3){ [needEasy,needMed,needHard] = CONFIG.mixWeights.low; } else if(level<=6){ [needEasy,needMed,needHard] = CONFIG.mixWeights.mid; } else if(level<=8){ [needEasy,needMed,needHard] = CONFIG.mixWeights.high; } else { [needEasy,needMed,needHard] = CONFIG.mixWeights.top; } function take(arr, n){ const a=[...arr]; const out=[]; while(out.length<n && a.length){ out.push(a.splice(Math.floor(Math.random()*a.length),1)[0]); } return out; } const picked = [...take(easy,needEasy), ...take(med,needMed), ...take(hard,needHard)]; while(picked.length<8 && tasks.length){ const t = tasks[Math.floor(Math.random()*tasks.length)]; if(!picked.includes(t)) picked.push(t); } return picked.slice(0,8); }
...
class Quiz{
  ...
  render(){ this.el.innerHTML=''; if(this.idx >= this.tasks.length){ this.renderSummary(); return; } const t = this.tasks[this.idx]; const head = el('div',{},[ el('h3',{},`Opgave ${this.idx+1} af ${this.tasks.length}`), el('p',{class:'small'},`Sværhedsgrad: ${t.difficulty}`) ]); const body = el('div',{},[ el('div',{},t.prompt) ]);
    let inputArea; if(t.type==='mcq'){ inputArea = el('div',{}, t.choices.map((c,i)=> el('button',{class:'button',onclick:()=> this.submit(i)}, `${String.fromCharCode(65+i)}. ${c}`)) ); } else if(t.type==='numeric'){ const placeholder = t.unit?`fx 78.5 ${t.unit}`:''; const inp = el('input',{type:'text',placeholder,style:'margin-right:8px'}); inputArea = el('div',{}, [inp, el('button',{class:'button',onclick:()=> this.submit(inp.value)},'Svar')]); } else { const inp = el('input',{type:'text',style:'margin-right:8px'}); inputArea = el('div',{}, [inp, el('button',{class:'button',onclick:()=> this.submit(inp.value.trim())},'Svar')]); }
  ...
  submit(ans){ const t = this.tasks[this.idx]; let given = ans; let givenUnit = null; if(t.type==='numeric'){ const p = parseNumericInput(ans); given = p.num; givenUnit = p.unit; } let ok = this.isCorrect(t, given); if(ok && CONFIG.strictUnits && t.type==='numeric' && t.unit){ if(!unitsMatch(t.unit, givenUnit)) ok = false; }
    ...
    let feedbackText = null; if(!ok){ if(t.type==='numeric'){ const ds = (t.distractors||[]).map(Number); let matched=null; for(const d of ds){ if(near(given,d)) { matched = d; break; } } if(matched!=null && t.feedback_map && t.feedback_map[String(matched)]){ const key = t.feedback_map[String(matched)]; feedbackText = FEEDBACK[key] || null; } } if(!feedbackText && CONFIG.strictUnits && t.type==='numeric' && t.unit){ feedbackText = FEEDBACK['unit_missing_or_wrong'] || 'Enhed mangler eller er forkert.'; } if(!feedbackText && t.feedback_key){ feedbackText = FEEDBACK[t.feedback_key] || null; } if(!feedbackText && t.hint){ feedbackText = t.hint; } }
...
function saveBadges(state){ localStorage.setItem(BADGES_KEY, JSON.stringify(state)); }
...
function renderBadgesStrip(){ const wrapId='badges-strip'; let node = document.getElementById(wrapId); if(!node){ node = el('div',{id:wrapId,class:'badges-strip'},[]); document.querySelector('header').appendChild(node); } const state = loadBadges(); const earnedIds = Object.keys(state.earned || {}); const icons = earnedIds.slice(0,6).map(id => el('span',{class:'badge-icon',title:BADGE_CATALOG[id]?.title || id}, BADGE_CATALOG[id]?.icon || '⭐')); node.innerHTML = ''; node.appendChild(el('div',{},[ el('span',{class:'small'},'Badges: '), ...icons, earnedIds.length>6 ? el('span',{class:'small',style:'margin-left:8px'}, `+${earnedIds.length-6} flere`) : null ])); const btn = el('a',{href:'#',class:'button',style:'margin-left:8px',onclick:(ev)=>{ev.preventDefault(); toggleBadgesPanel();}},'Se alle'); node.appendChild(btn); }
